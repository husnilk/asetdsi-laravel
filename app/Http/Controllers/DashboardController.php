...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $fromDate = (new Carbon('first day of Januari' . $year));
        // $toDate = (new Carbon)->now()->addDay(1)->endOfDay()->toDateString();

        $fromDate = Carbon::create($year, 1)->startOfMonth()->toDateString(); //returns 2020-03-01
        $toDate = Carbon::create($year, 12)->lastOfMonth()->toDateString(); //returns 2020-03-31
    ...
    public function indexPj(Request $request)
    {
        ...
        $newInventoryChart = [];
        $asetChart = array_filter(array_map(function ($data) use ($inventoryItemDate) {
            foreach ($inventoryItemDate as $itemMonth) {
                if ($data == $itemMonth->month) {
                    return json_decode(json_encode($itemMonth), true);
                }
